...
namespace srv_pkt
{
    void Config (OPacket *pkt, const WorldArea *area, const std::vector<Peer> &peers)
    {
        ...
        for (std::set<int>::const_iterator it = area->Zones.begin();
            it != area->Zones.end(); ++it)
            pkt->Write<uint16_t>(*it);

        pkt->Write<uint16_t>(peers.size());

        for (size_t i = 0; i < peers.size(); ++i)
        {
            pkt->Write<uint32_t>(peers[i].ID);
            pkt->Write(peers[i].host);
            pkt->Write(peers[i].port);
        }
    }

    void PeerState (OPacket *pkt, const uint8_t state)
    {
        pkt->WriteOpcode(SRV_PEER_STATE);
        pkt->Write<uint8_t>(state);
    }

    void SimulationAction (OPacket *pkt, const bool action)
    {
        pkt->WriteOpcode(SRV_SIMULATION_ACTION);
                          ...
                          const boost::shared_ptr<sso::CharacterInfo> &character)
    {
        ...
        pkt->WriteOpcode(SRV_CHARACTER_INSERT);
    ...
    {
        pkt->WriteOpcode(SRV_CHARACTER_ASSIGN);
        pkt->Write<uint32_t>(1);
        pkt->Write<uint32_t>(charID);
    }

    void CharacterAssign (OPacket *pkt, const std::vector<uint32_t> &IDs)
    {
        pkt->WriteOpcode(SRV_CHARACTER_ASSIGN);
        pkt->Write<uint32_t>(IDs.size());
    ...
    {
        pkt->WriteOpcode(SRV_CHARACTER_EXEC);
        pkt->Write<uint32_t>(charID);
    }

    void CharacterRemove (OPacket *pkt, const uint32_t charID)
    {
        pkt->WriteOpcode(SRV_CHARACTER_REMOVE);
        pkt->Write<uint32_t>(1);
        pkt->Write<uint32_t>(charID);
    }
