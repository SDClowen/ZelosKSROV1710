...
namespace sso
{
    InteractiveNPC::InteractiveNPC (const boost::shared_ptr<EntityProperty> &property)
        : ICharacter(property), m_FortressActive(false), m_FortressTax(0), m_TeleportID(0)
    {
    }
    ...
    uint8_t InteractiveNPC::getLevel () const
    {
    ...
    std::vector<uint8_t> InteractiveNPC::GetAvailableTasks (const uint8_t playerLv) const
    {
    ...
    bool InteractiveNPC::IsTaskAvailable (const uint8_t taskID, const uint8_t playerLv) const
    {
        for (std::vector<NPCTask>::const_iterator i = m_TaskList.begin(); i != m_TaskList.end(); ++i)
        {
            if (i->ID == taskID && playerLv >= i->MinLv)
                return true;
        }
