...
namespace srv
{
    ...
    IStateHandshake::IStateHandshake (const std::string &srvID, const boost::shared_ptr<IConnection> &conn)
    ...
    int IStateHandshake::OnHandshake (const IPacket &packet)
    {
        ...
        auth = boost::shared_static_cast<SROAuth,Authentication>(m_connection->getAuthentication());

        if (!auth)
            return MSG_ERROR;

        if (!(auth->getMode() & SECURITY_DH))
        ...
        auth->setKey((uint8_t*)priv_key,8);
        ...
        auth->encrypt((uint8_t*)srv_signature,(uint8_t*)srv_signature,8);
        ...
        OPacket pkt;
        srv_pkt::WriteHandshakeSignature(&pkt,srv_signature);
        m_connection->send(&pkt);

        Func_X_2((uint8_t*)m_base_key,S,0x3);

        auth->setKey((uint8_t*)m_base_key,8);

        return MSG_SUCCESS;
    }

    int IStateHandshake::OnHandshakeValidated (const IPacket &packet)
    {
        if (!packet.EndOfStream())
            return MSG_ERROR_SIZE;
