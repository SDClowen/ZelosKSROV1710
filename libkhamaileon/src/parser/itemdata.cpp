...
void ParserItemdata (const std::vector<std::string> &entries, boost::function<void (const ItemProperty&)> func)
{
    ...
    if (atoi(entries[idx++].c_str()) == 1)
    {
        ...
        property.refTransName = entries[idx++];
        ...
        property.Authority = 0;

        property.Authority |= atoi(entries[idx++].c_str()); /// STORAGEABLE

        ++idx;

        property.Authority |= (atoi(entries[idx++].c_str()) << 1); /// TRADEABLE

        /// SKIP 11 UNKNOWN COLUMNS
        idx += 11;
        ...
        property.pa.first = strtod(entries[idx++].c_str(),NULL);
        property.pa.second = strtod(entries[idx++].c_str(),NULL);
        property.pa_lv = strtod(entries[idx++].c_str(),NULL);
