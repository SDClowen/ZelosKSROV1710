...
uint16_t ItemProperty::GetTypeMask() const
{
    uint16_t mask = Types[0] | (Types[1] << 1) | (Types[2] << 2) |
                    (Types[3] << 5) | (Types[4] << 7) | (Types[5] << 11);

    return mask;
}

ITEM_EQUIP_POSITION ItemProperty::getDefaultPosition () const
{
    ITEM_EQUIP_POSITION pos = INVALID_SLOT;
