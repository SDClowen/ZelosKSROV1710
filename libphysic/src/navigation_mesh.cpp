...
namespace phy
{
    NavigationMesh::NavigationMesh (int16_t navID)
        : refID(navID), m_Texturemap(0), m_Tree(0)
    {
    }
            ...
            if (modelMgr)
            {
                ...
                for (it = citer->entities.begin(); it != citer->entities.end(); ++it)
                {
                    ...
                    if (model)
                    {
                        ...
                        std::pair<bool,uint8_t> objRet = model->resolveMotion(obj_path,&intersection);

                        if (objRet.first)
                        {
                            switch(objRet.second)
                            {
                    ...
                    {
    #if defined DEBUG
    //					std::cout << "ModelID = " << eit->refID << std::endl;
    #endif
                    }
                }
            }
